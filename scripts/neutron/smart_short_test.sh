...
curl -fsS --retry 5 -o /dev/null https://hc-ping.com/$PING_KEY/smart-short-test/start
...
# Log file
DATE=$(date +%Y%m%d)
LOGFILE="/srv/backup/Backup/logs/smart/short_test_$DATE.log"
...
curl -fsS --retry 5 -o /dev/null https://hc-ping.com/$PING_KEY/smart-short-test
